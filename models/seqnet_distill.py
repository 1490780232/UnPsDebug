...
class SeqNetDis(nn.Module):
    ...
    def inference(self, images, targets=None, query_img_as_gallery=False):
        ...
        if targets is not None and not query_img_as_gallery:
            ...
            return detections
    def inference_embeddings(self, images, targets=None):
        """
        query_img_as_gallery: Set to True to detect all people in the query image.
            Meanwhile, the gt box should be the first of the detected boxes.
            This option serves CBGM.
        ...
        if targets is not None:
            # query
            boxes = [t["boxes"] for t in targets]
            pids = [t["labels"] for t in targets]
            box_features = self.roi_heads.box_roi_pool(features, boxes, images.image_sizes)
            box_features = self.roi_heads.reid_head(box_features)
            embeddings, _ = self.roi_heads.embedding_head(box_features)
            return embeddings.cpu().detach(), torch.cat(pids).cpu()
...
class SeqRoIHeads(RoIHeads):
    ...
    def assign_targets_to_proposals(self, proposals, gt_boxes, gt_labels, indexes):
        matched_idxs = []
        labels = []
        match_indexes = []
        for proposals_in_image, gt_boxes_in_image, gt_labels_in_image, gt_indexes_in_image in zip(proposals, gt_boxes, gt_labels, indexes):
            ...
            if gt_boxes_in_image.numel() == 0:
                ...
                indexes_in_image = gt_indexes_in_image[clamped_matched_idxs_in_image]
                indexes_in_image = indexes_in_image.to(dtype=torch.int64)

                # Label background (below the low threshold)
                bg_inds = matched_idxs_in_image == self.proposal_matcher.BELOW_LOW_THRESHOLD
                labels_in_image[bg_inds] = 0
                indexes_in_image[bg_inds] = 0

                # Label ignore proposals (between low and high thresholds)
                ignore_inds = matched_idxs_in_image == self.proposal_matcher.BETWEEN_THRESHOLDS
                labels_in_image[ignore_inds] = -1  # -1 is ignored by sampler
                indexes_in_image[ignore_inds] = -1

            matched_idxs.append(clamped_matched_idxs_in_image)
            labels.append(labels_in_image)
            match_indexes.append(indexes_in_image)
        return matched_idxs, labels, match_indexes
    ...
    def select_training_samples(self,
        ...
        gt_indexes = [t["indexes"] for t in targets]
        ...
        matched_idxs, labels ,match_indexes = self.assign_targets_to_proposals(proposals, gt_boxes, gt_labels, gt_indexes)
        ...
        for img_id in range(num_images):
            ...
            match_indexes[img_id] = match_indexes[img_id][img_sampled_inds]
        ...
        return proposals, matched_idxs, labels, regression_targets, match_indexes
    ...
    def forward(self, features, proposals, image_shapes, targets=None, query_img_as_gallery=False):
        ...
        """
        if self.training:
            proposals, _, proposal_pid_labels, proposal_reg_targets, proposal_indexes = self.select_training_samples(
                proposals, targets
            )
        ...
        if self.training:
            ...
            boxes, _, box_pid_labels, box_reg_targets, box_indexes = self.select_training_samples(boxes, targets)
        else:
...
class NormAwareEmbedding(nn.Module):
    ...
    def __init__(self, featmap_names=["feat_res4", "feat_res5"], in_channels=[1024, 2048], dim=2048):
