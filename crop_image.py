...
root = "/home/lzy/UN_PS/UnPsDebug/data/cuhk_sysu"
# dataset = build_dataset("CUHKUnsupervised", "data/cuhk_sysu", transforms, "train")
...
dataset = build_dataset("CUHKUnsupervised", "data/cuhk_sysu", transforms, "gallery")
count =0
annotations = dataset.annotations
for i, ann in tqdm.tqdm(enumerate(dataset.annotations)):
    boxes = ann["boxes"]
    ...
    for i_box in range(len(indexes)):
        ...
        x1,y1,x2,y2 = boxes[i_box]
        pid = pids[i_box]
        cv2.imwrite(os.path.join(root, "crop_gallery_imgs",str(count).zfill(5)+".jpg"), im[y1:y2,x1:x2])
        count+=1

count =0
dataset = build_dataset("CUHKUnsupervised", "data/cuhk_sysu", transforms, "query")
annotations = dataset.annotations
for i, ann in tqdm.tqdm(enumerate(dataset.annotations)):
    boxes = ann["boxes"]
    # indexes = ann["indexes"]
    ...
    for i_box in range(len(pids)):
        x1,y1,x2,y2 = boxes[i_box]
        pid = pids[i_box]
        cv2.imwrite(os.path.join(root, "crop_query_imgs", str(count).zfill(5)+".jpg"), im[y1:y2,x1:x2])
        count+=1
