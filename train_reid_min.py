...
def main_worker(args):
    ...
    for epoch in range(args.epochs):
        ...
        # rerank_dist+= np.identity(N)*10
        # A = np.zeros((N, N))

        # min_distance = np.min(rerank_dist, axis=-1)
        # for i in range(N):
        #     rerank_dist[i][rerank_dist[i]>min_distance[i]] = 999


        #     A[i][min_distance[i]]=1
        # A = np.where(A>A.T, A, A.T)
