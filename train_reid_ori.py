...
def create_model(args):
    ...
    # model.load_state_dict(torch.load("/home/lzy/un_PS/SeqNet/ori/UnPsDebug/prw_baseline/checkpoint.pth.tar")['state_dict'], strict=True)
    return model

def build_test_loader_ps(args):
    transforms = build_transforms(is_train=False)
    if args.dataset == "cuhk_sysu":
    # gallery_set = build_dataset("CUHK-SYSU", "data/cuhk_sysu", transforms, "train")
        ...
        gallery_set = build_dataset("PRWUnsupervised", "data/PRW", transforms, "gallery")
        query_set = build_dataset("PRWUnsupervised", "data/PRW", transforms, "query")
...
def main_worker(args):
    ...
    gallery_loader, query_loader =build_test_loader_ps(args)
    ...
    for epoch in range(args.epochs):
        ...
        images_in = np.array([int(f.split('/')[-1].split("_")[0]) for f, _, _ in sorted(dataset.train)])
        images_in = np.expand_dims(images_in, axis=0)
        ...
        if epoch == 0:
            ...
            cluster = DBSCAN(eps=eps, min_samples=args.min_sample, metric='precomputed', n_jobs=-1)
        ...
        # thro
        # # np.ex
        # rerank_dist[images_in==images_in.T]=999
        # rerank_dist_back = rerank_dist+np.identity(N)*10
        # min_distance = np.min(rerank_dist_back,axis=-1)
        # for i in range(N):
        #     rerank_dist[i][rerank_dist[i]> min_distance[i]] = 10
        # rerank_dist = np.where(rerank_dist<rerank_dist.T, rerank_dist, rerank_dist.T)
        pseudo_labels, num_clust, req_c = FINCH(np.asarray(features), image_in = images_in, initial_rank=None, req_clust=None, distance='cosine', verbose=True)
        # import pdb; pdb.set_trace()
        pseudo_labels = pseudo_labels[:,0]
        # pseudo_labels = cluster.fit_predict(rerank_dist)
        # rerank_dist = cosine_distances(features,features)

        # pseudo_labels = labels #cluster.fit_predict(rerank_dist)
        # pseudo_labels = np.array(pseudo_labels)
        # print(sum(pseudo_labels==5555),"555555555")
        # pseudo_labels[pseudo_labels==5555]=-1

         # else: np.sort(rerank_dist[17])[:20] 
        #     pseudo_labels = np.arange(0, features.shape[0])
