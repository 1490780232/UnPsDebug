...
def create_model(args):
    ...
    model.load_state_dict(torch.load("/home/lzy/UN_PS/UnPsDebug/instance_8_test_0.55/model_best.pth.tar")['state_dict'])
    print("loaded models")
    return model
...
def main_worker(args):
    ...
    model.eval()
    with torch.no_grad():
        imag = Image.open("/home/lzy/UN_PS/UnPsDebug/data/PRW/crop_query_imgs/00479_00000.jpg")
        image = test_transformer(imag).unsqueeze(0)
        print(image.shape)
        inputs = to_torch(image).cuda()
    ...
    for epoch in range(args.epochs):
        with torch.no_grad():
            ...
            features, _ = extract_features(model, test_loader, print_freq=50)
            exit()

            features = torch.cat([features[f].unsqueeze(0) for f, _, _ in sorted(dataset.train)], 0)
            torch.save(features,"features_instance.pt", )
